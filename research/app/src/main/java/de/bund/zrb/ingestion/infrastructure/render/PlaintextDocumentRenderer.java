...
/**
 * Renders a Document to Plaintext format (compact, minimal overhead).
 */
public class PlaintextDocumentRenderer implements DocumentRenderer {

    private final RendererRegistry registry;

    public PlaintextDocumentRenderer(RendererRegistry registry) {
    ...
    public String render(Document document) {
        ...
        for (Block block : document.getBlocks()) {
            BlockRenderer renderer = registry.findBlockRenderer(RenderFormat.PLAINTEXT, block);
            if (renderer != null) {
                renderer.render(block, out);
            } else {
                // Fallback: try to get text somehow
                out.append("[Block: ").append(block.getType()).append("]\n\n");
            }
        }
